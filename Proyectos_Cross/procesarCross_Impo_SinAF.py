...
def listaTomarR1(df_cli_filtrado,df_ds_AF,df_gp_rs):
    ...
    df_gp = df_gp[
    df_gp["CLAVE PED"].str.startswith(("1-IN-", "1-V1-"), na=False) &
...
def ejecutar_proceso(dfs,df_ped):
    ...
    # df_cliente_impo=listaCliente(df_cli,"EXPO", ["RT", "V1"])
    df_gp_rs=listaTomar(df_cliente_impo,df_cli,df_ds_AF)

    #2 CREACION DEL DATAFRAME DE DE R1 PARA CORRECCION DE IMPO/EXPO
    df_cliente_impo_r1=listaCliente(df_cli,"IMPO", ["R1"])
    # df_cliente_impo_r1=listaCliente(df_cli,"EXPO", ["R1"])
    df_gp_rs_r1,df_gp_rs_vf=listaTomarR1(df_cliente_impo_r1,df_ds_AF,df_gp_rs)
